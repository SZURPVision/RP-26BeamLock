...
cv::Mat euler_zyx_to_rotation(double yaw_deg, double pitch_deg, double roll_deg)
{
    constexpr double k_deg_to_rad = 3.14159265358979323846 / 180.0;
    const double a = yaw_deg * k_deg_to_rad;
    const double b = pitch_deg * k_deg_to_rad;
    const double c = roll_deg * k_deg_to_rad;
    ...
    R.at<double>(0, 0) = ca * cb;
    R.at<double>(0, 1) = ca * sb * sc - sa * cc;
    R.at<double>(0, 2) = ca * sb * cc + sa * sc;
    R.at<double>(1, 0) = sa * cb;
    R.at<double>(1, 1) = sa * sb * sc + ca * cc;
    R.at<double>(1, 2) = sa * sb * cc - ca * sc;
    R.at<double>(2, 0) = -sb;
    R.at<double>(2, 1) = cb * sc;
    R.at<double>(2, 2) = cb * cc;
...
bool read_guide_transform(const std::string& path, cv::Mat& tvec, cv::Mat& rotation)
{
    ...
    // 3. Read ZYX Euler Angles (yaw pitch roll) in degrees
    double yaw_deg = 0.0, pitch_deg = 0.0, roll_deg = 0.0;
    if (std::getline(file, line))
    {
        std::istringstream iss(line);
        if (!(iss >> yaw_deg >> pitch_deg >> roll_deg))
        {
            std::cerr << "Failed to read ZYX Euler angles from line." << std::endl;
            return false;
        }
    }
    else
    {
        std::cerr << "Unexpected end of file while reading Euler angles." << std::endl;
        return false;
    }

    rotation = euler_zyx_to_rotation(yaw_deg, pitch_deg, roll_deg);
    file.close();

    std::cout << "tvec: " << tvec.t() << std::endl;
    std::cout << "ZYX Euler (yaw,pitch,roll) deg: " << yaw_deg << " " << pitch_deg << " " << roll_deg << std::endl;
    std::cout << "rotation:\n" << rotation << std::endl;
